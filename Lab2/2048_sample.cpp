...
public:
	...
	state select_best_move(const board& b) const {
		...
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)) {
				...
				board table = move->after_state();
				for(int i = 0; i < 16; i++){
					if(table.at(i) == 0){
						space.push_back(i);
					}
				}	
				int num_space = space.size();
				float expect = 0;
				for(int i=0;i<num_space;i++){
					board buf1 = table;
					board buf2 = table;
					buf1.set(space[i],1);
					expect += (0.9 * estimate(buf1));
					buf2.set(space[i],2);
					expect += (0.1 * estimate(buf2));
					// delete buf;
				}
				expect /= num_space;
				// std::cout << expect << std::endl;
				move->set_value(move->reward() + expect);
	...
	void update_episode(std::vector<state>& path, float alpha = 0.1) const {
        ...
        for(path.pop_back();path.size();path.pop_back()){
            ...
            // TD = r + V(s'[t+1]) - V(s'[t])
            float TD_error = move.reward() + target - estimate(move.before_state());
            target = update(move.before_state(), alpha * TD_error);
        }
	}
...
void save_scorecsv(std::vector<float>& scores,std::string path){
    std::ofstream file;
    file.open(path);
    file<<"score\n";
...
int main(int argc, const char* argv[]) {
	...
	tdl.add_feature(new pattern({ 9, 10, 11, 13, 14, 15 }));

	// restore the model from file
	tdl.load("weight-300000.bin");
	...
	// tdl.save("weight-100000-0.3.bin");
	// save score.csv
    // save_scorecsv(scores,"score-100000-0.3.csv");
	return 0;
}