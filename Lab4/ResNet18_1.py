...
def train(train_loader, test_loader, model, loss_fn, optimizer, epochs, num_class, device, name):
    ...
    df['Train(with pretraining)'] = acc_train
    df['Test(with pretraining)'] = acc_test
...
def test(test_loader, model, loss_fn, device, num_class):
    ...
    correct /= size
    correct *= 100
    print(f"Test: Accuracy: {(correct):.2f}%, loss: {test_loss:.4f} \n")
...
if __name__ == '__main__':
    ...
    model_1 = initialize_model(
        "Resnet18", num_classes, feature_extract, use_pretrained=True).to(device)

    params_to_update = model_1.parameters()

    optimizer = optim.SGD(params_to_update, lr=learning_rate,
                          momentum=Momentum, weight_decay=Weight_decay)
    df_1 = train(train_loader, test_loader, model_1, loss_fn,
                 optimizer, epochs, num_classes, device, "model_Resnet18_1")
    df_1.to_csv("ResNet18_1.csv", index=False)
    confusion_matrix, acc = test(
        test_loader, model_1, loss_fn, device, num_classes)
    figure = plot_confusion_matrix(confusion_matrix)
    figure.savefig('ResNet18 (Pretrained weights).png')
    ...
    #     "Resnet18", num_classes, feature_extract, use_pretrained=False).to(device)
    ...
    #              optimizer, epochs, num_classes, device, "model_Resnet18_0")
    ...
    # figure = plot(df_0, df_1, 'Result Comparison(ResNet18)')
