...
if __name__ == '__main__':
    ...
    model_1 = initialize_model(
        "Resnet50", num_classes, feature_extract, use_pretrained=True).to(device)
    ...
    df_1 = train(train_loader, test_loader, model_1, loss_fn,
                 optimizer, epochs, num_classes, device, "model_Resnet50_1")
    df_1.to_csv("ResNet50_1.csv", index=False)
    confusion_matrix, acc = test(
        test_loader, model_1, loss_fn, device, num_classes)
    figure = plot_confusion_matrix(confusion_matrix)
    figure.savefig('ResNet50 (Pretrained weights).png')
